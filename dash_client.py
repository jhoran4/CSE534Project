...
from adaptation import basic_dash, basic_dash2, weighted_dash, netflix_dash, randomized_dash
...
def start_playback_smart(dp_object, domain, playback_type=None, download=False, video_segment_duration=None):
    ...
    randomized_state = "INITIAL"
    ra_gamma = config_dash.RANDOMIZED_GAMMA
    ra_alpha = config_dash.RANDOMIZED_ALPHA
    decision = None
    majority = None
    expected_download = None
    # Start playback of all the segments
    print("=================")
    pprint.pprint(dp_list[1])
    enum_obj = enumerate(dp_list, dp_object.video[current_bitrate].start)
    for obj in enum_obj:
        pprint.pprint(obj)
    print("=================")
    for segment_number, segment in enumerate(dp_list, dp_object.video[current_bitrate].start):
        ...
        if segment_number == dp_object.video[bitrate].start:
            ...
            elif playback_type.upper() == "RANDOMIZED":
                config_dash.LOG.info("Playback is RANDOMIZED")
                
                # Same as in the SARA policy, obtain a Harmonic Weighted Mean
                # Experiment with higher sample count to make randomness more reliable
                if not weighted_mean_object:
                    weighted_mean_object = WeightedMean(config_dash.RANDOMIZED_SAMPLE_COUNT)
                    config_dash.LOG.debug("Initializing the weighted Mean object")
                if segment_number < len(dp_list) - 1 + dp_object.video[bitrate].start:
                    #try:
                        # Different from SARA policy:
                        # Use a series of next segment sizes based on the LOOKAHEAD constant
                        segment_size_list = []
        ...
        previous_download_rate = segment_size/segment_download_time
        ...
        if (playback_type.upper() == "SMART" or playback_type.upper() == "RANDOMIZED") and weighted_mean_object:
...
def main():
    ...
    print('=============')
    for key, video in dp_object.video.items():
        print(key)
        pprint.pprint(vars(video))
    print('=============')
    print('=============')
    print("Video Segment Duration: " + str(video_segment_duration))
    print("Domain: " + domain)
    print("DOWNLOAD: " + str(DOWNLOAD))
    pprint.pprint(vars(dp_object.video[88783]))
    print('=============')
    ...
    elif "randomized" in PLAYBACK.lower():
        config_dash.LOG.critical("Started Randomized-DASH Playback")
        start_playback_smart(dp_object, domain, "RANDOMIZED", DOWNLOAD, video_segment_duration)
    else:
